...
- Does not use ISMEAR = -5 for static jobs by default, since this can negatively impact forces/stresses for metallic systems.
...
- Added AQCat25-compatible recipe

### Fixed

- Fixed K-point handling for oc20_static_job
...
- Overhauled quacc's logging utilities to be more user friendly

## [1.5.8]

### Changed

- Rename VASP_CUSTODIAN_VASP_ERROR_EXCLUDE to VASP_CUSTODIAN_ERROR_EXCLUDE

## [1.5.7]

### Added

- MD recipes for MLIPs
...
- Internal refactoring of decorator handling for various engines

### Fixed

- Fixed passing of `Job` objects in jobflow workflows to avoid calling `.output`

## [1.5.1]

### Changed

- Restored `RosenSetGGA.yaml` for internal use
...
- Added the missing `sdftd3` parameter to set BJ damping when ivdw=15

## [1.4.0]

### Added

- AIMD workflows for VASP
...
- More comprehensive logging with regards to VASP input parameter swaps and recommendations

### Changed

- The VASP INCAR co-pilot will no longer automatically set ISMEAR = -5 for all static calculations
- A subset of critical VASP INCAR swaps will override user inputs by default
...
- The folders generated by quacc recipes now follows a hierarchical folder structure if the setting `AUTODISCOVER_DIR`
  is set to `true`
...
- Fixed `copy_files` so it ignores Custodian error folders
...
- Fixed file I/O when Jobflow is used with `run_locally(create_folders=True)`
...
- Added OMC static recipe
- Added OMat static recipe
