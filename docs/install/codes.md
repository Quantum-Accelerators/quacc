...
If you plan to use tblite with quacc, you will need to install the tblite interface with ASE support. This can be done via `pip install quacc[tblite]` if on Linux. If you are using a Mac, it can be installed via `conda install -c conda-forge tblite-python`.
