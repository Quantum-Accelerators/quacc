...
=== "Jobflow"
    ...
    def workflow(atoms):
        ...
        # Define Job 2, which takes the output of Job 1 as input
        return static_job(job1["atoms"])  # (1)!
    ...
    result = responses[final_job.uuid][1].output
    print(result)
    ```

    1. Quacc automatically passes the output reference between its decorated jobs, so the workflow remains workflow-engine agnostic.
...
=== "Ray"
    ...
    1. Quacc wraps Ray `ObjectRef`s in a `RayFuture` proxy that supports `__getitem__` so that recipe code which indexes into job results (e.g. `future1["atoms"]`) lazily creates a downstream Ray task and you do not need to call `#!Python ray.get` between dependent jobs.
    ...
    !!! Tip "Modifying the Decorator of a Pre-Made Job"
        ...
        relax_job_ = redecorate(relax_job, job(num_cpus=1))  # (1)!
        ```
...
=== "Jobflow"
    ...
    result1 = responses[job1.uuid][1].output
    result2 = responses[job2.uuid][1].output
...
=== "Ray"
    ...
    !!! Tip "Selectively Modifying Job Decorators in a Pre-Made Flow"
        ...
        If you want to modify the decorators of select jobs in a pre-made workflow, such as to adjust the requested resources of a given function, you can use the `job_decorators` keyword argument:

        ```python
        bulk_to_slabs_flow(atoms, job_decorators={"static_job": job(num_cpus=1)})
        ```

        As a shorthand, all of the decorators can be modified at once using the "all" key:

        ```python
        bulk_to_slabs_flow(atoms, job_decorators={"all": job(num_cpus=1)})
        ```
