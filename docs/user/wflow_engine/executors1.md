...
=== "Covalent"
    ...
    **Configuring Executors**
    ...
    4. You generally want each quacc job to be run in its own unique working directory to ensure files don't overwrite one another, so  `CREATE_UNIQUE_DIR` should be set to `True`.
