...
=== "Parsl"
    ...
    Let's consider a scenario where we want to run concurrent VASP jobs that each use run on a full CPU node of 128 cores. We will run two concurrent VASP jobs in a Slurm allocation.
