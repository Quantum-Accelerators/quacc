...
An example is shown below for storing the results in a MongoDB via the {obj}`quacc.utils.db.covalent_to_db` function. For assistance with setting up a MongoDB of your own, refer to the ["MongoDB Setup"](../../install/advanced/config_db.md) section of the installation instructions.

```python
from maggma.stores import MongoStore
from quacc.utils.db import covavlent_to_db
...
If you're not using a workflow manager, you can still store your results in a database of your choosing. For a given recipe, you can store the output summary dictionary in your database using the {obj}`quacc.utils.db.results_to_db` function, as shown in the example below.
