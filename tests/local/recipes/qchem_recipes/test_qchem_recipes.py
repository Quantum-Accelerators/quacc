...
def test_static_job_v5(tmp_path, test_atoms):
    os.chdir(tmp_path)
...
def test_relax_job_v4(tmp_path, test_atoms):
    os.chdir(tmp_path)
    with pytest.raises(ValueError):
...
def test_ts_job_v4(tmp_path, test_atoms):
    os.chdir(tmp_path)
    with pytest.raises(ValueError):
...
def test_irc_job_v2(tmp_path, test_atoms):
    os.chdir(tmp_path)
    with pytest.raises(ValueError):
