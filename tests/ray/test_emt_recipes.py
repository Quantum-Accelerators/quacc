...
def test_copy_files(tmp_path, monkeypatch):
    ...
    @flow
    def myflow(atoms):
        result1 = (relax_job(atoms)).result()
        return relax_job(
...
def test_ray_phonon_flow(tmp_path, monkeypatch):
    ...
    assert future.result()["results"]["thermal_properties"]["temperatures"].shape == (
        101,
    )


def test_ray_phonon_flow_multistep(tmp_path, monkeypatch):
    pytest.importorskip("phonopy")
    pytest.importorskip("seekpath")
    from quacc.recipes.emt.phonons import phonon_flow
    ...
    monkeypatch.chdir(tmp_path)
    atoms = bulk("Cu")
    relaxed = (relax_job(atoms)).result()
