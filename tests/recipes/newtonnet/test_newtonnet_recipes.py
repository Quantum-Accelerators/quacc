...
def test_freq_job():
    ...
    assert output["vib"]["results"]["vib_freqs_raw"][0] == pytest.approx(-292.9037211526164)
    assert output["vib"]["results"]["vib_freqs_raw"][-1] == pytest.approx(4071.5043841913816)
    assert output["vib"]["results"]["vib_freqs"][0] == pytest.approx(1863.4478302128284)
    assert output["vib"]["results"]["vib_freqs"][-1] == pytest.approx(4071.5043841913816)
    ...
    assert output["thermo"]["results"]["entropy"] == pytest.approx(0.0019582532509612592)
    assert output["thermo"]["results"]["gibbs_energy"] == pytest.approx(-9.294281257052218)
    ...
    assert output["vib"]["results"]["vib_energies_raw"][0] == pytest.approx(-1.5636131383185185e-09)
    assert output["vib"]["results"]["vib_energies_raw"][-1] == pytest.approx(0.40918939605228777)
    assert output["vib"]["results"]["vib_energies"][0] == pytest.approx(0.09310183598706114)
    assert output["vib"]["results"]["vib_energies"][-1] == pytest.approx(0.40918939605228777)
    assert output["vib"]["results"]["n_imag"] == 0
    assert output["vib"]["results"]["imag_vib_freqs"] == []
