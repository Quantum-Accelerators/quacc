...
def test_presets():
    ...
    atoms_fullpath = Vasp(atoms, preset=os.path.join(DEFAULT_CALCS_DIR, "BulkRelaxSet"))
    atoms = Vasp(atoms, preset="BulkRelaxSet")
    ...
    atoms = Vasp(atoms, xc="rpbe", preset="SlabRelaxSet")
    assert atoms.calc.xc.lower() == "rpbe"
    assert atoms.calc.string_params["algo"] == "fast"
    assert atoms.calc.exp_params["ediff"] == 1e-5
    assert atoms.calc.int_params["isif"] == 2

    atoms = Vasp(atoms, xc="scan", preset="MPScanRelaxSet")
...
def test_autodipole():
    ...
    atoms = Vasp(atoms, auto_dipole=True)
    ...
    atoms = Vasp(atoms, auto_dipole=True, idipol=2)
    assert atoms.calc.bool_params["ldipol"] is True
    assert atoms.calc.int_params["idipol"] == 2
    assert np.array_equal(atoms.calc.list_float_params["dipol"], com)

    atoms = Vasp(atoms, preset="SlabRelaxSet")
    assert atoms.calc.bool_params["ldipol"] is True
    assert atoms.calc.int_params["idipol"] == 3
    assert np.array_equal(atoms.calc.list_float_params["dipol"], com)

    atoms = Vasp(atoms, preset="SlabRelaxSet", idipol=2)
    assert atoms.calc.bool_params["ldipol"] is True
    assert atoms.calc.int_params["idipol"] == 2
    assert np.array_equal(atoms.calc.list_float_params["dipol"], com)

    atoms = Vasp(atoms, auto_dipole=False, preset="SlabRelaxSet", idipol=2)
...
def test_ediff_per_atom():
    atoms = bulk("Cu") * (2, 2, 2)
    atoms = Vasp(atoms, ediff_per_atom=1e-4)
...
def test_magmoms():
    ...
    atoms = bulk("Zn") * (2, 2, 1)
    atoms[-1].symbol = "Fe"
    atoms = Vasp(atoms, preset="BulkRelaxSet", mag_default=2.5)
    ...
    atoms = bulk("Cu") * (2, 2, 1)
    atoms[-1].symbol = "Fe"
    atoms = Vasp(atoms, preset="MPScanRelaxSet")
    ...
    atoms = deepcopy(ATOMS_MAG)
    atoms = Vasp(atoms, preset="BulkRelaxSet", mag_cutoff=2.0)
    assert atoms.has("initial_magmoms") is True
    assert np.all(atoms.get_initial_magnetic_moments() == 0)
    ...
    atoms = bulk("Cu")
    atoms = Vasp(atoms, preset="BulkRelaxSet", copy_magmoms=False)
    ...
    atoms = deepcopy(ATOMS_MAG)
    atoms = prep_next_run(atoms)
    atoms = Vasp(atoms, preset="BulkRelaxSet", mag_cutoff=10.0)
    assert atoms.has("initial_magmoms") is True
    assert np.all(atoms.get_initial_magnetic_moments() == 0)
...
def test_unused_flags():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, preset="BulkRelaxSet", potim=1.5, nsw=0)
    ...
    atoms = Vasp(atoms, ldau=False, ldauprint=2)
...
def test_lasph():
    ...
    atoms = Vasp(atoms, xc="rpbe")
    assert atoms.calc.bool_params["lasph"] is None

    atoms = Vasp(atoms, xc="m06l")
    assert atoms.calc.bool_params["lasph"] is True

    atoms = Vasp(atoms, xc="m06l", lasph=False)
    assert atoms.calc.bool_params["lasph"] is True

    atoms = Vasp(atoms, xc="hse06")
    assert atoms.calc.bool_params["lasph"] is True

    atoms = Vasp(atoms, xc="beef-vdw")
    assert atoms.calc.bool_params["lasph"] is True

    atoms = Vasp(atoms, ldau_luj={"Cu": {"L": 2, "U": 5, "J": 0.0}})
    assert atoms.calc.bool_params["lasph"] is True


def test_lmaxtau():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, lasph=True)
...
def test_algo():
    ...
    atoms = Vasp(atoms, xc="m06l", algo="fast")
    assert atoms.calc.string_params["algo"] == "all"
...
def test_kpar():
    ...
    atoms = Vasp(atoms, kpts=[2, 2, 1], kpar=4)
    assert atoms.calc.int_params["kpar"] == 4

    atoms = Vasp(atoms, kpar=4)
...
def test_isym():
    ...
    atoms = Vasp(atoms, isym=2)
    assert atoms.calc.int_params["isym"] == 2

    atoms = Vasp(atoms, isym=0)
    assert atoms.calc.int_params["isym"] == 0

    atoms = Vasp(atoms, xc="hse06", isym=2)
    assert atoms.calc.int_params["isym"] == 3

    atoms = Vasp(atoms, isym=2, nsw=100)
    assert atoms.calc.int_params["isym"] == 0

    atoms = Vasp(atoms, xc="hse06", isym=2, nsw=100)
    assert atoms.calc.int_params["isym"] == 0
...
def test_ncore():
    ...
    atoms = Vasp(atoms, ncore=16)
    assert atoms.calc.int_params["ncore"] == 1

    atoms = Vasp(atoms, npar=16)
    assert atoms.calc.int_params["ncore"] == 1
    assert atoms.calc.int_params["npar"] is None

    atoms *= (2, 2, 2)
    atoms = Vasp(atoms, ncore=4)
    assert atoms.calc.int_params["ncore"] == 4

    atoms = Vasp(atoms, ncore=4, lhfcalc=True)
    assert atoms.calc.int_params["ncore"] == 1

    atoms = Vasp(atoms, npar=4, lhfcalc=True)
    assert atoms.calc.int_params["ncore"] == 1
    assert atoms.calc.int_params["npar"] is None
...
def test_ismear():
    ...
    atoms = Vasp(atoms, nsw=10)
    assert atoms.calc.int_params["ismear"] is None

    atoms = Vasp(atoms, ismear=-5, nsw=10)
    assert atoms.calc.int_params["ismear"] == 1
    assert atoms.calc.float_params["sigma"] == 0.1

    atoms = Vasp(atoms, ismear=-5, nsw=0)
    assert atoms.calc.int_params["ismear"] == 0

    atoms = Vasp(atoms, kpts=(10, 10, 10), ismear=-5, nsw=0)
    assert atoms.calc.int_params["ismear"] == -5

    atoms = Vasp(atoms, ismear=0, nsw=10)
    assert atoms.calc.int_params["ismear"] == 0

    atoms = Vasp(atoms, nedos=3001, nsw=0)
    assert atoms.calc.int_params["ismear"] == 0

    atoms = Vasp(atoms, ismear=-5, nedos=3001, nsw=0)
    assert atoms.calc.int_params["ismear"] == 0

    atoms = Vasp(atoms, kpts=(10, 10, 10), nedos=3001, nsw=0)
    assert atoms.calc.int_params["ismear"] == -5

    atoms = Vasp(atoms, auto_kpts={"line_density": 100}, ismear=1)
    assert atoms.calc.int_params["ismear"] == 0
    assert atoms.calc.float_params["sigma"] == 0.01

    atoms = Vasp(atoms, auto_kpts={"line_density": 100}, ismear=0, sigma=1e-3)
    assert atoms.calc.int_params["ismear"] == 0
    assert atoms.calc.float_params["sigma"] == 1e-3

    atoms = Vasp(atoms, auto_kpts={"line_density": 100}, ismear=-5)
    assert atoms.calc.int_params["ismear"] == 0

    atoms = Vasp(atoms, kspacing=1.0, ismear=-5)
    assert atoms.calc.int_params["ismear"] == 0
    assert atoms.calc.float_params["sigma"] == 0.05

    atoms = Vasp(atoms, nsw=0, kspacing=1.0, ismear=1, sigma=0.1)
    assert atoms.calc.int_params["ismear"] == 1
    assert atoms.calc.float_params["sigma"] == 0.1

    atoms[0].symbol = "H"
    atoms = Vasp(atoms, kpts=(10, 10, 10), ismear=-5, nsw=10)
    assert atoms.calc.int_params["ismear"] == -5


def test_laechg():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, nsw=10, laechg=True)
    assert atoms.calc.bool_params["laechg"] is None

    atoms = Vasp(atoms, laechg=True)
    assert atoms.calc.bool_params["laechg"] is True

    atoms = Vasp(atoms, nsw=0, laechg=True)
    assert atoms.calc.bool_params["laechg"] is True

    atoms = Vasp(atoms, nsw=0, laechg=False)
...
def test_ldauprint():
    ...
    atoms = Vasp(atoms, ldau=True)
    assert atoms.calc.int_params["ldauprint"] == 1

    atoms = Vasp(atoms, ldau=True, ldauprint=0)
    assert atoms.calc.int_params["ldauprint"] == 1

    atoms = Vasp(atoms, ldau=False, ldauprint=1)
    assert atoms.calc.int_params["ldauprint"] is None
...
def test_lreal():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, lreal=True, nsw=0)
    assert atoms.calc.special_params["lreal"] is False

    atoms = Vasp(atoms, lreal=True, nsw=10)
...
def test_lorbit():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, ispin=2)
    assert atoms.calc.int_params["lorbit"] == 11

    atoms = bulk("Cu")
    atoms = Vasp(atoms, ispin=1)
...
def test_setups():
    ...
    atoms = bulk("Cu")
    atoms = Vasp(atoms, setups="minimal", preset="MPScanRelaxSet")
    assert (
...
def test_kpoint_schemes():
    atoms = bulk("Cu")
    atoms = Vasp(atoms, kpts=[1, 1, 1], preset="BulkRelaxSet")
    assert atoms.calc.kpts == [1, 1, 1]

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"grid_density": 1000}, gamma=False)
    assert atoms.calc.kpts == [10, 10, 10]
    assert atoms.calc.input_params["gamma"] is False

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"grid_density": 1000})
    assert atoms.calc.kpts == [10, 10, 10]
    ...
    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"grid_density": 1000}, gamma=True)
    assert atoms.calc.kpts == [10, 10, 10]
    assert atoms.calc.input_params["gamma"] is True

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"reciprocal_density": 100})
    assert atoms.calc.kpts == [12, 12, 12]

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"max_mixed_density": [100, 1000]})
    assert atoms.calc.kpts == [12, 12, 12]

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"length_density": [50, 50, 1]})
    assert atoms.calc.kpts == [20, 20, 1]

    atoms = bulk("Cu")
    atoms = Vasp(atoms, auto_kpts={"line_density": 100})
