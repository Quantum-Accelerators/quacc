...
def test_create_mrcc_eint_blocks(embedded_adsorbed_cluster,element_info):
    ...
    mrcc_blocks = create_mrcc_eint_blocks(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=True)

    mrcc_blocks_nocp = create_mrcc_eint_blocks(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=False)


    reference_block_collated = {'adsorbate_slab': {'float': [21.0, 2.0, -2.0, -2.0, 2.0, -8.9536039173],
  'string': ['basis_sm=atomtype', 'def2/JK', 'O']},
 'adsorbate': {'float': [8.0], 'string': ['basis_sm=atomtype', '3,4,5,6,7,8']},
 'slab': {'float': [21.0, 2.0, -2.0, -2.0, 2.0, -8.9536039173],
  'string': ['basis_sm=atomtype', 'def2/JK', 'O']}}
    
    reference_block_nocp_collated = {'adsorbate_slab': {'float': [21.0, 2.0, -2.0, -2.0, 2.0, -8.9536039173],
  'string': ['basis_sm=atomtype', 'def2/JK', 'O']},
 ...
 'slab': {'float': [19.0,
  ...
  'string': ['basis_sm=atomtype', 'no-basis-set', 'Mg']}}
    ...
    generated_block_collated = {system: {'float': [], 'string': []} for system in ['adsorbate_slab','adsorbate','slab']}
    generated_block_nocp_collated = {system: {'float': [], 'string': []} for system in ['adsorbate_slab','adsorbate','slab']}

    for system in ['adsorbate_slab','adsorbate','slab']:
        generated_block_collated[system]['float'] = [
        float(x)
        for x in mrcc_blocks[system].split() if x.replace(".", "", 1).replace("-", "", 1).isdigit()
    ][::300]
        generated_block_collated[system]['string'] = [
        x
        for x in mrcc_blocks[system].split()if not x.replace(".", "", 1).replace("-", "", 1).isdigit()][::50]


        generated_block_nocp_collated[system]['float'] = [
        float(x)
        for x in mrcc_blocks_nocp[system].split() if x.replace(".", "", 1).replace("-", "", 1).isdigit()
        ][::300]
        generated_block_nocp_collated[system]['string'] = [
        x
        for x in mrcc_blocks_nocp[system].split() if not x.replace(".", "", 1).replace("-", "", 1).isdigit()][::50]


        assert_equal(generated_block_collated[system]['string'],reference_block_collated[system]['string'])
        assert_allclose(generated_block_collated[system]['float'],reference_block_collated[system]['float'],rtol=1e-05,atol=1e-07)

        assert_equal(generated_block_nocp_collated[system]['string'],reference_block_nocp_collated[system]['string'])
        assert_allclose(generated_block_nocp_collated[system]['float'],reference_block_nocp_collated[system]['float'],rtol=1e-05,atol=1e-07)
    


def test_generate_mrcc_basis_ecp_block(embedded_adsorbed_cluster,element_info):
    generated_mrcc_blocks = generate_mrcc_basis_ecp_block(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=True)

    generated_mrcc_blocks_nocp = generate_mrcc_basis_ecp_block(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=False)

    reference_mrcc_blocks_collated = {'adsorbate_slab': ['basis_sm=atomtype',
  'no-basis-set',
  'no-basis-set',
  'aug-cc-pVDZ',
  'no-basis-set',
 ...
 'adsorbate': ['basis_sm=atomtype', 'aug-cc-pVDZ', 'def2/JK', 'cc-pVDZ/C']}
    ...
    reference_mrcc_blocks_nocp_collated = {'adsorbate_slab': ['basis_sm=atomtype',
  'no-basis-set',
  'no-basis-set',
  'aug-cc-pVDZ',
  'no-basis-set',
 ...
 'adsorbate': ['basis_sm=atomtype', 'aug-cc-pVDZ/C']}
    ...
    generated_mrcc_blocks_nocp_collated = {system: [] for system in ['adsorbate_slab','slab','adsorbate']}
    generated_mrcc_blocks_collated = {system: [] for system in ['adsorbate_slab','slab','adsorbate']}    
    for system in ['adsorbate_slab','adsorbate','slab']:
        generated_mrcc_blocks_collated[system] = generated_mrcc_blocks[system].split()[::10]
        generated_mrcc_blocks_nocp_collated[system] = generated_mrcc_blocks_nocp[system].split()[::10]
        
        assert_equal(generated_mrcc_blocks_collated[system],reference_mrcc_blocks_collated[system])
        assert_equal(generated_mrcc_blocks_nocp_collated[system],reference_mrcc_blocks_nocp_collated[system])
...
def test_create_mrcc_atomtype_basis(embedded_adsorbed_cluster,element_info):
    ...
    ecp_region = embedded_adsorbed_cluster[[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24]]

    generated_basis_block_without_ecp = create_mrcc_atomtype_basis(quantum_region=quantum_region,element_basis_info= {element: element_info[element]['ri_cwft_basis'] for element in element_info})
    generated_basis_block_with_ecp = create_mrcc_atomtype_basis(quantum_region=quantum_region,element_basis_info= {element: element_info[element]['ri_cwft_basis'] for element in element_info},ecp_region=ecp_region)
...
def test_generate_mrcc_coords_block(embedded_adsorbed_cluster,element_info):
    ...
    mrcc_blocks = generate_mrcc_coords_block(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=True)

    mrcc_blocks_nocp = generate_mrcc_coords_block(embedded_cluster=embedded_adsorbed_cluster,quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24],element_info=element_info,include_cp=False)

    reference_block_collated = {'adsorbate_slab': {'float': [21.0,
   -2.12018425659,
   -2.12018425659,
   -0.04367284424,
   0.0,
 ...
 'adsorbate': {'float': [8.0, -2.12018425659, -2.12018425659],
  'string': ['charge=0', 'C', 'O']},
 'slab': {'float': [21.0,
   -2.12018425659,
   -2.12018425659,
  ...
  'string': ['charge=-8', 'C', 'O', 'Mg', 'Mg', 'Mg']}}
    ...
    reference_block_nocp_collated = {'adsorbate_slab': {'float': [21.0,
   -2.12018425659,
   -2.12018425659,
   -0.04367284424,
   0.0,
 ...
 'slab': {'float': [19.0,
  ...
  'string': ['charge=-8', 'Mg', 'O', 'Mg', 'Mg']}}
    ...
    for system in ['adsorbate_slab','adsorbate','slab']:
        ...
        generated_block_collated[system]['string'] = [
        x
        for x in mrcc_blocks[system].split()if not x.replace(".", "", 1).replace("-", "", 1).isdigit()][::5]
    ...
    for x in mrcc_blocks_nocp[system].split() if x.replace(".", "", 1).replace("-", "", 1).isdigit()
        ...
        generated_block_nocp_collated[system]['string'] = [
        x
        for x in mrcc_blocks_nocp[system].split() if not x.replace(".", "", 1).replace("-", "", 1).isdigit()][::5]
...
def test_generate_mrcc_point_charge_block(embedded_adsorbed_cluster):
    generated_point_charge_block = generate_mrcc_point_charge_block(embedded_cluster=embedded_adsorbed_cluster, quantum_cluster_indices=[0, 1, 2, 3, 4, 5, 6, 7], ecp_region_indices=[8, 9, 10, 11, 12, 13, 14, 15, 20, 21, 22, 23, 24]).split()

    generated_point_charge_block_shortened = [float(x) for x in generated_point_charge_block[5::70]]
 ...
 43.73621546646]
    ...
    assert_allclose(generated_point_charge_block_shortened,reference_point_charge_block_shortened,rtol=1e-05,atol=1e-07)
