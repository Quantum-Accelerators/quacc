...
with change_settings(
    ...
    def test_relax_job(tmp_path, monkeypatch):
        ...
        assert not np.array_equal(
            output["atoms"].get_positions(), atoms.get_positions()
        )
    ...
    def test_freq_job(tmp_path, monkeypatch):
        ...
        assert output["results"]["vib_energies"][0] == pytest.approx(
            -0.09441402482739979
        )
        assert output["results"]["vib_energies"][-1] == pytest.approx(
            0.3925829460532815
        )
        assert output["results"]["n_imag"] == 1
        assert output["results"]["imag_vib_freqs"] == pytest.approx(
            [-761.5004719152678]
        )
    ...
    def test_ts_job_with_custom_hessian(tmp_path, monkeypatch):
        ...
        # Call the function
        output = ts_job(
            atoms, use_custom_hessian=use_custom_hessian, opt_params=opt_params
        )
    ...
    def test_irc_job_with_default_args(tmp_path, monkeypatch):
        ...
        assert output["freq_job"]["results"]["energy"] == pytest.approx(
            -9.517354091813969
        )
    ...
    def test_quasi_irc_job_with_default_args(tmp_path, monkeypatch):
        ...
        assert output["irc_job"]["results"]["energy"] == pytest.approx(
            -9.517354091813969
        )
        assert output["results"]["energy"] == pytest.approx(-9.517354091813969)
        assert output["freq_job"]["results"]["energy"] == pytest.approx(
