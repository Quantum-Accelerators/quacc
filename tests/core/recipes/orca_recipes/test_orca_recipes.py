...
def test_static_job(tmp_path, monkeypatch):
    ...
    assert (
        ...
        == "def2-tzvp engrad normalprint wb97m-d4 xyzfile"
    )
...
def test_static_job_parallel(tmp_path, monkeypatch):
    ...
    assert (
        output["parameters"]["orcasimpleinput"]
        == "def2-svp engrad normalprint wb97m-d4 xyzfile"
    )
...
def test_relax_job(tmp_path, monkeypatch):
    ...
    assert (
        output["parameters"]["orcasimpleinput"]
        == "def2-tzvp normalprint opt wb97m-d4 xyzfile"
    )
    ...
    output = relax_job(
        ...
        charge=-2,
        spin_multiplicity=3,
        orcasimpleinput=["def2-svp", "#def2-tzvp", "#wb97m-d4", "hf"],
        orcablocks=["%scf maxiter 300 end"],
        nprocs=2,
    )
    assert output["molecule_metadata"]["natoms"] == len(atoms)
