...
def test_phonon_flow_fixed(tmp_path, monkeypatch):
    ...
    output_fixed = phonon_flow(full_system, run_relax=True, min_lengths=5.0, fixed_indices=[0, 1])
    ...
    output_wrong = phonon_flow(full_system, run_relax=True, min_lengths=5.0, fixed_indices=[0, 2])
    assert output["results"]["mesh_properties"]["frequencies"] != pytest.approx(
