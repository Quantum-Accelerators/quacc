...
@pytest.mark.parametrize(
    ...
    [
        "DefaultPBESet",
        Path(presets.__file__).parent / "DefaultPBESet.yaml",
        str(Path(presets.__file__).parent / "DefaultPBESet.yaml"),
    ],
)
...
def test_li_sv4(caplog):
    ...
    atoms[0].symbol = "Li"
    with caplog.at_level(WARNING):
        Vasp(atoms, isif=3, nsw=2, preset="DefaultPBESet")
    assert "Pulay" in caplog.text
...
def test_kspacing():
    ...
    calc = Vasp(atoms, kspacing=0.1, preset="DefaultPBESet")
...
def test_magmoms(atoms_mag, atoms_nomag, atoms_nospin):
    ...
    atoms = bulk("Zn") * (2, 2, 1)
    atoms[-1].symbol = "Fe"
    calc = Vasp(atoms, preset="DefaultPBESet", preset_mag_default=2.5)
    atoms.calc = calc
    ...
    atoms = deepcopy(atoms_mag)
    calc = Vasp(atoms, preset="DefaultPBESet", mag_cutoff=2.0)
    atoms.calc = calc
    assert np.all(atoms.get_initial_magnetic_moments() == 0)
...
def test_prep_magmoms2(atoms_mag, atoms_nomag, atoms_nospin):
    ...
    atoms = deepcopy(atoms_mag)
    atoms = prep_next_run(atoms, move_magmoms=True)
    calc = Vasp(atoms, preset="DefaultPBESet", mag_cutoff=10.0)
    atoms.calc = calc
    assert atoms.has("initial_magmoms") is True
    assert np.all(atoms.get_initial_magnetic_moments() == 0)
...
def test_unused_flags():
    atoms = bulk("Cu")
    calc = Vasp(atoms, preset="DefaultPBESet", potim=1.5, nsw=0)
...
def test_setups():
    ...
    atoms = bulk("Cu")
    calc = Vasp(atoms, setups=FILE_DIR / "test_setups.yaml", preset="DefaultPBESet")
    assert calc.parameters["setups"]["Cu"] == "_sv"

    atoms = bulk("Cu")
    calc = Vasp(atoms, setups="setups_vasp_recommended.yaml", preset="DefaultPBESet")
    assert calc.parameters["setups"]["Cu"] == ""

    atoms = bulk("Cu")
    calc = Vasp(atoms, setups="minimal", preset="DefaultPBESet")
...
def test_kpoint_schemes():
    ...
    atoms = bulk("Cu")
    calc = Vasp(atoms, preset="DefaultPBESet", kspacing=0.1)
    assert calc.kpts is None
    assert calc.float_params["kspacing"] == 0.1
    ...
    atoms = bulk("Cu")
    calc = Vasp(atoms, preset="DefaultPBESet", pmg_kpts={"kppa": 1000}, gamma=False)
    assert calc.kpts == [10, 10, 10]
    assert calc.input_params["gamma"] is False
...
def test_preset_override():
    ...
    calc = Vasp(atoms, preset="DefaultPBESet", efermi=None)
