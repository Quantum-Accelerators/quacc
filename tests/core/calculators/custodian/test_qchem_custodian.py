...
from quacc.calculators.qchem.custodian import run_custodian

pytest.importorskip("openbabel")


def mock_custodian_run(*args, **kwargs):
    """Instead of running Custodian, we will mock it to return True when .run() is
    called."""
...
def patch_custodian_run(monkeypatch):
    """Monkeypatch the Custodian.run() function so that it doesn't actually launch
    Custodian during a test."""
