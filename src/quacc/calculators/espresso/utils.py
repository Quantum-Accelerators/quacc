...
def parse_pp_and_cutoff(config: dict, atoms) -> dict[str, Any]:
    ...
    if 'pseudopotentials' in config:
        ...
        for element in unique_elements:
            wfc_cutoff.append(
                pp_dict[element]['cutoff_wfc']
            )
            rho_cutoff.append(
                pp_dict[element]['cutoff_rho']
            )
            pseudopotentials[element] = \
                config['pseudopotentials'][element]['filename']
    else:
        return {}
    tmp_input_data = {'system': 'ecutwfc': max(wfc_cutoff), 'ecutrho': max(rho_cutoff)}
