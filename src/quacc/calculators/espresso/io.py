...
def read_espresso_ph(fd):
    ...
    def _read_modes(idx):
        ...
        modes = []
        while len(modes) == 0 or bool(
                re.match(r'^\s*\(', fdo_lines[idx + n])):
    ...
    def _read_eqpoints(idx):
        ...
        eqpoints = np.loadtxt(fdo_lines[idx + 2:idx + 2 + n_star],
                              usecols=(1, 2, 3))
        return eqpoints
    ...
    properties = {NKPTS: _read_kpoints,
                  DIEL: _read_epsil,
                  BORN: _read_born,
