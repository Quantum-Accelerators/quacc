...
class RunAndSummarize:
    """
    Run and summarize a Gaussian calculation.
    """
    ...
    def calculate(self) -> cclibSchema:
        ...
        final_atoms = run_calc(
            self.input_atoms, geom_file=LOG_FILE, copy_files=self.copy_files
        )
    ...
    def _prepare_calc(self) -> None:
        ...
        self.input_atoms.calc = Gaussian(
            command=GAUSSIAN_CMD, label=_LABEL, **calc_flags
        )
