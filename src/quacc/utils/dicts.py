...
class Remove:
    ...
    `None` is a valid value for many keyword arguments. Also, using `()`
    as the sentinel value is not robust because its value changes every time
    it is instantiated, which means an `()` provided by the user locally
    will not match an `()` instantiated on the remote machine.
    """
