...
For workflow engines that serialize tasks to separate processes (Redun,
Jobflow, Parsl), the context is captured at call-site via special kwargs
(``_quacc_ctx``, ``_quacc_dir``) and restored on the worker before the
tracked function executes.
"""
...
class ContextNode(MSONable):
    ...
    def __eq__(self, other):
        return (
            isinstance(other, ContextNode)
            and self.name == other.name
            and self.node_type == other.node_type
        )

    def as_dict(self):
        """Serialize to a plain dict for JSON/BSON storage."""
        return {"name": self.name, "node_type": self.node_type.value}

    @classmethod
    def from_dict(cls, d: dict[str, Any]):
        """Reconstruct a ContextNode from its serialized dict form."""
        return cls(d["name"], NodeType(d["node_type"]))
